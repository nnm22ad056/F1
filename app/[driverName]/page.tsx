// /app/driver/[driverName]/page.tsx
// /app/[driverName]/page.tsx
"use client"
...
export default function DriverPage  ({ params }: DriverPageProps)  {
  const { driverName } = use(params);
  
  const firstNamer  = driverName.split("-")[0];
  const name = firstNamer.charAt(0).toUpperCase() + firstNamer.slice(1);
  // const lastName  = driverName.split("-")[1];
  const [driver, setDriver] = useState(null); // To store driver details
  const [firstName, setFirstName] = useState(name); // Example value for testing
  const [error, setError] = useState(null); // To handle errors

  useEffect(() => {
    const fetchDriverDetails = async () => {
      try {
        const response = await axios.post('/api/drivername', {
          firstName: firstName,
        });
        setDriver(response.data.driverData); // Store the driver data
      } catch (err) {
                ...
                <div className="mt-6 sm:mt-8 lg:mt-0">
                  ...
                  <div className="pt-5">
                    <ul className="">
                      ...
                      <li>
                        <strong>Podiums:</strong> {driver.podiums} 
                      </li>
                      <li>
                        <strong>Championships:</strong> {driver.championships}
                      </li>
                      <li>
                        <strong>Car:</strong> {driver.car}
                      </li>
                      <li>
                        <strong>Team:</strong> {driver.manager.company}
                      </li>
                      <li>
